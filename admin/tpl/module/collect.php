...
elseif($method=='cjjson'){
	...
	if($pre=='vod') {
        ...
        foreach ($json['list'] as $one) {
            ...
            $rc = false;
            $d_id = $one['vod_id'];
            $d_name = format_vodname(strip_tags($one['vod_name']));
            $d_name = str_replace("'", "''", $d_name);
            $d_remarks = strip_tags($one['vod_remarks']);
            $d_remarks = str_replace("'", "''", $d_remarks);
            $d_state = intval($one['vod_serial']);
            $d_type = $xt == '0' ? $one['type_id'] : $flag . $one['type_id'];
            $d_type = intval($bindcache[$d_type]);
            $d_starring = strip_tags($one['vod_actor']);
            $d_starring = str_replace("'", "''", $d_starring);
            $d_directed = strip_tags($one['vod_director']);
            $d_directed = str_replace("'", "''", $d_directed);
            $d_pic = strip_tags($one['vod_pic']);
            $d_pic = str_replace("'", "''", $d_pic);
            $d_time = $one['vod_time'];
            $d_year = intval($one['vod_year']);
            $d_area = strip_tags($one['vod_area']);
            $d_area = str_replace("'", "''", $d_area);
            $d_lang = strip_tags($one['vod_lang']);
            $d_lang = str_replace("'", "''", $d_lang);
            $d_content = strip_tags($one['vod_content']);
...
EOT;
        ...
        foreach ($json['list'] as $one) {
            ...
            $rc = false;
            $a_id = $one['art_id'];
            $a_name = format_vodname(strip_tags($one['art_name']));
            $a_name = str_replace("'", "''", $a_name);
            $a_remarks = strip_tags($one['art_remarks']);
            $a_remarks = str_replace("'", "''", $a_remarks);
            $a_subname = intval($one['art_subname']);
            $a_subname = str_replace("'", "''", $a_remarks);
            $a_type = $xt == '0' ? $one['type_id'] : $flag . $one['type_id'];
            $a_type = intval($bindcache[$a_type]);
            $a_tag = strip_tags($one['art_tag']);
            $a_tag = str_replace("'", "''", $a_tag);
            $a_from = strip_tags($one['art_from']);
            $a_from = str_replace("'", "''", $a_from);
            $a_author = strip_tags($one['art_author']);
            $a_author = str_replace("'", "''", $a_author);
            $a_pic = strip_tags($one['art_pic']);
            $a_pic = str_replace("'", "''", $a_pic);
            $a_time = $one['art_time'];
            $a_content = strip_tags($one['art_content']);
